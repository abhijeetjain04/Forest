...
CEnvironment::CEnvironment(int ScreenWidth, int ScreenHeight, float *passed_CameraX, float *passed_CameraY, CSDL_Setup* passed_csdl_setup)
{
	...
	for(int i = 0; i < 4; i++)
	{
		for (int j = 0; j < 7; j++)
		{
			grass[i][j] = new CSprite(csdl_setup->GetRenderer(),"data/grass03.png", ScreenWidth * i, ScreenHeight*j, ScreenWidth, ScreenHeight, CameraX, CameraY, CCollisionRectangle ());
			gress_w = ScreenWidth * i;
			gress_h = ScreenHeight * j;
		}
	}
	...
	std::cout<<gress_w<<"\t"<<gress_h<<std::endl;
}
...
void CEnvironment::Update()
{
	...
	// To on/off the level creation mode
	if (csdl_setup->GetMainEvent()->type == SDL_KEYDOWN)
	{
